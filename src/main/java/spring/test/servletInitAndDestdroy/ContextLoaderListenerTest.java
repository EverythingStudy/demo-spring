...
/**
 * @Description ContextLoaderListenerTest实现servletContextListener接口对ServletContext容器监听
 * 1。web应用初始化时ServletContext创建完成，同时设置属性，spring也同样利用监听器监听web应用容器，初始化时将webApplicationContext放入ServletContext中
 * 2.只要监听了servletContext在web应用销毁时或者停止Tomcat时，自动执行contextDestroyed(ServletContextEvent sce)
